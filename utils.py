...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        q_out = self.q(x)
        k_out = self.k(context if context is not None else x)
        v_out = self.v(context if context is not None else x)

        q_out = q_out.view(N, T, h, self.d_qk).transpose(1, 2)
        k_out = k_out.view(N, -1, h, self.d_qk).transpose(1, 2)
        v_out = v_out.view(N, -1, h, self.d_v ).transpose(1, 2)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        x = self.layerNorm1(x + self.multiHeadAttention(x, mask=mask))
...
class Seq2SeqDecoderBlock(nn.Module):
    def __init__(self, d_model, d_qk, d_v, n_att, max_length):
        ...
        self.self_attn = CasualMaskMultiHeadAttention(d_model, d_qk, d_v, n_att, max_length)
        self.ln1 = nn.LayerNorm(d_model)
        
        self.cross_attn = MultiHeadAttention(d_model, d_qk, d_v, n_att)
    ...
    def forward(self, x, enc_out, pad_mask=None, enc_mask=None):
        x = self.ln1(x + self.self_attn(x, pad_mask))

        x = self.ln2(x + self.cross_attn(x, context=enc_out, mask=enc_mask))
